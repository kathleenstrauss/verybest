class VenuesController < ApplicationController
  def index
    ...
    @venues = @q.result(:distinct => true).includes(:favorites, :neighboorhood).page(params[:page]).per(10)
  ...
  def create_row
    ...
    @venue.neighboorhood_id = params.fetch("neighboorhood_id")
  ...
  def create_row_from_neighborhood
    ...
    if @venue.valid?
      ...
      redirect_to("/neighborhoods/#{@venue.neighboorhood_id}", notice: "Venue created successfully.")
    else
      render("venue_templates/new_form_with_errors.html.erb")
    end
  end
  ...
  def destroy_row_from_neighboorhood
    @venue = Venue.find(params.fetch("id_to_remove"))

    @venue.destroy

    redirect_to("/neighborhoods/#{@venue.neighboorhood_id}", notice: "Venue deleted successfully.")
  end
